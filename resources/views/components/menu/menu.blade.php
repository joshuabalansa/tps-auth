...
@section('content')
    ...
    <style>
        .swing-in-top-fwd {
            -webkit-animation: .5s cubic-bezier(.175, .885, .32, 1.275) both swing-in-top-fwd;
            animation: .5s cubic-bezier(.175, .885, .32, 1.275) both swing-in-top-fwd
        }
    ...
    <div class="flex justify-evenly w-full items-center p-2">
        <div class="flex justify-center items-center w-full">
            <img src="{{ asset('logo/logo.png') }}" class="h-12 mr-3" alt="Rabbi Hole Cafe" />
            <h1 class="mb-4 text-2xl font-extrabold text-gray-900 dark:text-white md:text-4xl lg:text-5xl"><span
                    class="text-transparent bg-clip-text bg-gradient-to-r to-emerald-600 from-sky-400">The Rabbit Hole</span>
                Cafe</h1>
            @if ($cart_items_count != 0)
                <a href="{{ route('cart.index') }}"
                    class="text-white rounded-full p-3 hover:bg-blue-600 fixed bottom-4 right-4 md:bottom-8 md:right-8 lg:bottom-12 lg:right-12 md:p-8 md:shadow-lg lg:p-10  z-50">
                    <lord-icon src="https://cdn.lordicon.com/mfmkufkr.json" trigger="loop" delay="1000"
    ...
    <div class="flex items-center justify-center py-3 md:py-3 flex-wrap">
        ...
        @foreach ($categories as $category)
            <a href="{{ route('menu.selectedCategory', $category->category_id) }}"
            ...
            <div class="flex flex-wrap gap-4 px-4 sm:px-6 md:px-8 lg:px-20 justify-center">
                @foreach ($menus as $menu)
                    <a href="{{ route('menu.show', $menu->id) }}"
                        class="card-link hover:shadow-lg swing-in-top-fwd w-full sm:w-1/2  md:w-1/3 lg:w-1/4">
                        <div class="relative bg-white rounded-lg shadow-md flex flex-col justify-center	items-center">
                            <img loading="eager" class="object-contain object-center w-48 h-48 object-cover rounded-t-lg"
                                src="{{ asset('uploads/' . $menu->image) }}" alt="Category 1">
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class="p-6 text-center">
                        ...
                        <button data-modal-hide="popup-modal" type="button"
                            class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">Order
                            a menu</button>
                    </div>
                </div>
            </div>
